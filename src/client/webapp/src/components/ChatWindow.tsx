...
import type { OutgoingMessage } from "../hooks/useChatSocket";
...
export default function ChatWindow() {
  ...
  // lấy hàm gửi WS từ context
  const { sendWSMessage } = useOutletContext<AppOutletContext>();
  ...
  const handleSend = () => {
    ...
    if (replyingTo) {
      // Send reply - only via WebSocket, server will add it
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    await sendFileMessage(conv.id, file, "file", sendWSMessage);
  ...
  const handleImageSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    await sendFileMessage(conv.id, file, "image", sendWSMessage);
    e.target.value = ""; // Reset input
  };
